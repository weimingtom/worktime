 ...
 * Time: 15:19
 */
public class TimeRegistrationEditStartTimeActivity extends RoboSherlockActivity {
    ...
    private void validateInput() {
        if (timeRegistration.getStartTime().getTime() == newStartTime.getTimeInMillis()) {
            //The date and time haven't changed, so no custom_validation is required and result is fine.
            finish();
        }
    ...
    private boolean validateAgainstLowerLimit(Calendar time, Calendar limit) {
        ...
        if (time.getTime().after(limit.getTime())) {
            Log.d(getApplicationContext(), LOG_TAG, "The new time is greater than or equal to the limit, custom_validation ok!");
            return true;
        }
    ...
    private boolean validateAgainstHigherLimit(Calendar time, Calendar limit) {
        ...
        if(time.getTime().before(limit.getTime())) {
            Log.d(getApplicationContext(), LOG_TAG, "The new time is less than the limit, custom_validation ok!");
            return true;
        }
